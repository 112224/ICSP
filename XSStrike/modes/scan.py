...
def scan(target, paramData, encoding, headers, delay, timeout, skipDOM, find, skip):
    ...
    response = requester(target, {}, headers, POST, delay, timeout).text
    ...
    url = getUrl(target, POST)
    logger.debug('Url to scan: {}'.format(url))
    params = getParams(target, paramData, POST)
    logger.debug_json('Scan parameters:', params)
    if find:
        params = arjun(url, POST, headers, delay, timeout)
    if not params:
        logger.error('No parameters to test.')
        quit()
    WAF = wafDetector(
        url, {list(params.keys())[0]: xsschecker}, headers, POST, delay, timeout)
    if WAF:
    ...
    for paramName in params.keys():
        ...
        response = requester(url, paramsCopy, headers, POST, delay, timeout)
        ...
        efficiencies = filterChecker(
            url, paramsCopy, headers, POST, delay, occurences, timeout, encoding)
