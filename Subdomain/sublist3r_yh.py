...
class portscan():
    ...
    def port_scan(self, host, ports):
        ...
        self.lock.release()
        if len(openports) > 0:
            self.subdomains_with_openports.append("%s open ports:%s"%(host,', '.join(openports)))
            print("%s%s%s - %sFound open ports:%s %s%s%s" % (G, host, W, R, W, Y, ', '.join(openports), W))
        else:
            self.subdomains_with_openports.append("%s open ports:None"%(host))
